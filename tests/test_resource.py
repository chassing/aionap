...
async def test_request_kwargs(httpbin):
    # be sure default user-agent is set
    async with aionap.API(httpbin.url) as api:
        resource = getattr(api, "user-agent")
        resp = await resource.get()
        assert resp["user-agent"]

    async with aionap.API(httpbin.url, request_kwargs={"skip_auto_headers": ["user-agent"]}) as api:
        resource = getattr(api, "user-agent")
        resp = await resource.get()
        assert not resp["user-agent"]
